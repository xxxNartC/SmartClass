...
<html>
    ...
    <h1 style="text-align: center;">Lesson List Management</h1>
    <!-- Add Subject button -->
    <a href="addlesson?chapter_id=${requestScope.chapter.chapter_id}&course_id=${requestScope.course.course_id}" class="add-lesson-btn">Add Lesson</a>
    <!--Table nhung du lieu-->
    <table id="tablelesson" class="display" style="width:100%">
        <thead>
            <tr>
    ...
    <script type="text/javascript">
        ...
        function loadData() {
            $.ajax({
                //            lay du lieu tu url 
                url: "/smartclass/lessonmanage?chapter_id=${requestScope.chapter.chapter_id}&course_id=${requestScope.course.course_id}",
                type: 'POST',
                dataType: 'json',
                success: function (data) {
                    console.log(data); // Kiểm tra dữ liệu
                    if (!table) { // Nếu table chưa được khởi tạo
                        table = $('#tablelesson').DataTable({
                            ...
                            columns: [
                                ...
                                {data: null,
                                    render: function (data) {
                                        return '<a href="editlesson?lesson_id=' + data.lesson_id + '&chapter_id=${requestScope.chapter.chapter_id}&course_id=${requestScope.course.course_id}">'
                                                + '<button class="btn btn-success btn-sm">Edit</button></br>'
                                                + '</a>'
                                                + '<a href="deletelesson?lesson_id=' + data.lesson_id + '&chapter_id=${requestScope.chapter.chapter_id}&course_id=${requestScope.course.course_id}">'
                                                + '<button onclick="return confirm(\' Do you want to delete this LESSON \')" class="btn btn-danger btn-sm">Delete</button>'
                                                + '</a>';
                                    }
                                }
                            ]
