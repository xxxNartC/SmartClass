...
<html>
 ...
 <c:if test="${sessionScope.user.role_id == '3'}">
        ...
        <div class="container">
        <h1 style="text-align: center;margin-top: 16px">Update Course</h1>
        <div style="margin: 0 auto; width: 1000px">
            <h1 class="error-message">${mess}</h1>

            <form action="update-course" method="post" class="add-subject-form">
                <div class="form-group">
                    <input value="${c.getSubject_id()}" type="hidden" class="form-control" id="subjecId" name="subjecId" required>
                    <input value="${c.getCourse_id()}" type="hidden" class="form-control" id="courseId" name="courseId" required>
                    <label for="subjectName">Course No:</label>
                    <input value="${c.getCourse_no()}" type="text" class="form-control" id="courseNo" name="courseNo" required>
                </div>
                
                <div class="form-group">
                    <label for="subjectName">Course Name:</label>
                    <input value="${c.getCourse_name()}" type="text" class="form-control" id="courseName" name="courseName" required>
                </div>
                
                 <div class="form-group">
                    <label for="image">Image:</label>
                    <input value="${c.image}" class="form-control" id="image" name="image" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="status" name="status">
                        <option value="1" <c:if test="${c.status == true}">selected</c:if>>Active</option>
                        <option value="0" <c:if test="${c.status == false}">selected</c:if>>InActive</option>
                    </select>
                </div>

                <button type="submit" class="btn btn-primary">Save</button>
                <a href="coursemanage?subject_id=${c.getSubject_id()}" class="btn btn-dark">Cancel</a>
            </form>
        </div>
